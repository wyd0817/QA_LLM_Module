...
class GradioLlmInterface:
    ...
    def initialize_interface(self, mode):
        ...
        chatbot = gr.Chatbot(label="DART-LLM")
        state = gr.State({"file_path": file_path, "initial_messages": initial_messages, "mode": mode, "llm_handler": llm_handler})
    ...
    async def predict(self, input, state):
        ...
        messages = [(full_history[i]["content"], full_history[i+1]["content"]) for i in range(len(initial_messages), len(full_history)-1, 2)]
